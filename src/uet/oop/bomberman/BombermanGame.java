...
public class BombermanGame extends Application {
    ...
    private static List<Entity> enemyObjects = new ArrayList<>();
        ...
        scene.setOnKeyPressed(new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent event) {
                ...
                switch (event.getCode()) {
                    case RIGHT:
                        if (Collision.checkCollision(bomberman.getX() +5, bomberman.getY(), unTravelableList))
                            bomberman.moveRight();
                        break;

                    case DOWN:
                        if (Collision.checkCollision(bomberman.getX(), bomberman.getY()+ 5, unTravelableList))
                            bomberman.moveDown();
                        break;
    ...
    private void renderEndGame() {
        ...
        gc.fillText(result, Math.round(canvas.getWidth() /2)-300, Math.round(canvas.getHeight() /2 -20));
        gc.fillText("Your Score: "+score, Math.round(canvas.getWidth() /2)-350, Math.round(canvas.getHeight() /2)+100);
    }
    ...
    private void handleBomberCollideEnemy() {
        for (Entity entity : entities) {
            if (entity instanceof Balloon) {
                ...
                if (Collision.checkCollision(baloon.getX(), baloon.getY(), bomberman.getX(), bomberman.getY()))
                    bomberman.dead();
            }

            if (entity instanceof Oneal) {
                Oneal oneal =(Oneal) entity;
                if (Collision.checkCollision(oneal.getX(), oneal.getY(), bomberman.getX(), bomberman.getY()))
                    bomberman.dead();
            }
        }
    }
